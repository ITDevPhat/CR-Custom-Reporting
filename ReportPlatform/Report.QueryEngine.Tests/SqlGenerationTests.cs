...
public sealed class SqlGenerationTests
{
    ...
    public void MultiHopProductCategoryPath_ShouldCompileExpectedJoins()
    {
        ...
        AssertSqlContains(result.Sql.Sql, "JOIN DimProductCategory pc ON ps.ProductCategoryKey = pc.ProductCategoryKey");
        AssertSqlContains(result.Sql.Sql, "GROUP BY pc.EnglishProductCategoryName");
    }
    ...
    [Fact]
    public void MultiHopSortByDimension_ShouldUseIntermediateAlias()
    {
        var harness = BuildProductHierarchyHarness();
        var result = harness.Compile(Request(
            rows: ["dimproductcategory.englishproductcategoryname"],
            values: ["metric.total_sales"],
            sort: [new SortRequest { Field = "dimproductcategory.englishproductcategoryname", Direction = "DESC" }]));

        AssertSqlContains(result.Sql.Sql, "ORDER BY [EnglishProductCategoryName] DESC");
        AssertSqlContains(result.Sql.Sql, "JOIN DimProductSubcategory ps ON p.ProductSubcategoryKey = ps.ProductSubcategoryKey");
    }

    [Fact]
    public void MultipleShortestPaths_ShouldFailAmbiguousRelationshipPath()
    {
        var harness = new QueryEngineTestHarness(model =>
        {
            ...
            model.Fields.Add(new SemanticField { DatasetId = "sales", FieldId = "dimproductsubcategory.name", TableId = "DimProductSubcategory", PhysicalTable = "DimProductSubcategory", PhysicalColumn = "Name", DisplayName = "Name", DataType = "nvarchar", Role = "dimension", Grain = "product_subcategory", SemanticType = "category", DefaultAggregation = "none", Format = "general", IsHidden = false, IsDraggable = true });
            model.Relationships.Add(new SemanticRelationship { DatasetId = "sales", RelationshipId = "rel_ps_direct", FromTableId = "FactSales", FromColumn = "ProductSubcategoryKey", ToTableId = "DimProductSubcategory", ToColumn = "ProductSubcategoryKey", JoinType = "INNER", Cardinality = "N:1", CrossFilterDirection = "single", IsActive = true, IsPrimary = true, Source = "database_fk", Confidence = 1.0m, Status = "active" });
            model.Relationships.Add(new SemanticRelationship { DatasetId = "sales", RelationshipId = "rel_prod_ps", FromTableId = "DimProduct", FromColumn = "ProductSubcategoryKey", ToTableId = "DimProductSubcategory", ToColumn = "ProductSubcategoryKey", JoinType = "INNER", Cardinality = "N:1", CrossFilterDirection = "single", IsActive = true, IsPrimary = true, Source = "database_fk", Confidence = 1.0m, Status = "active" });
            return model;
        });

        var act = () => harness.Compile(Request(rows: ["dimproductsubcategory.name"], values: ["metric.total_sales"]));
        act.Should().Throw<SemanticQueryValidationException>()
            .Where(ex => ex.Errors.TryGetValue("errorCode", out var code) && code.Contains("AMBIGUOUS_RELATIONSHIP_PATH"));
    }
