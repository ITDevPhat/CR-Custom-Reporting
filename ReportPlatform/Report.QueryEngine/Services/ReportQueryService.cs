...
public sealed class ReportQueryService
{
    ...
    public async Task<ComprehensiveQueryResponse> ExecuteAsync(VisualQueryRequest request, CancellationToken ct)
    {
        ...
        try
        {
            var model = await _modelStore.LoadAsync(request.DatasetId, ct); var context = new ValidationContext(request, model);
    ...
    public async Task<CompiledReportQuery> CompileForExportAsync(
    VisualQueryRequest request,
    CancellationToken ct)
    {
        var model = await _modelStore.LoadAsync(request.DatasetId, ct);

        var compiled = CompileQuery(request, model, ct);

        return new CompiledReportQuery
        {
            ConnectionId = request.ConnectionId,
            Sql = compiled.Sql.Sql,
            Parameters = compiled.Sql.Parameters,
            ExpectedColumns = compiled.ExpectedColumns,
            Warnings = []
        };
    }
